/*
 Blog: https://www.pvk.ca/Blog/2017/12/22/appnexus-common-framework-its-out-also-how-to-print-integers-faster/
 Discussion: https://news.ycombinator.com/item?id=15997680
 Source: https://github.com/appnexus/acf/blob/master/src/an_itoa.c
 License: https://github.com/appnexus/acf/blob/master/LICENSE

 Code is modified for benchmark.
 Require __uint128_t and __builtin_ctz.
 */

#if (defined(__GNUC__) || defined(__clang__)) && (defined(__LP64__) || defined(_LP64))
...
encode_hundreds(uint32_t hi, uint32_t lo)
{
     ...
     */
    return (uint32_t)(tens + ((merged - 10UL * tens) << 8));
}
 ...
 */
static AN_CC_INLINE char *_itoa_u32_an(uint32_t x, char *out) {
    uint64_t buf;
    uint32_t x_div_PO4;
    uint32_t x_mod_PO4;
    uint32_t x_div_PO8;
 ...
 */
static AN_CC_INLINE char *_itoa_u64_an(uint64_t x, char *out) {
    uint64_t x_div_PO4;
    uint64_t x_mod_PO4;
    uint64_t x_div_PO8;
    uint64_t buf;
    ...
    if (AN_CC_LIKELY(x < PO2)) {
        return itoa_hundred(out, (uint32_t)x);
    }

    if (AN_CC_LIKELY(x < PO4)) {
        return itoa_ten_thousand(out, (uint32_t)x);
    }
    ...
    if (AN_CC_LIKELY(x < PO10)) {
        out = itoa_hundred(out, (uint32_t)x_div_PO8);
        memcpy(out, &buf, 8);
        return out + 8;
    }
    ...
    if (AN_CC_LIKELY(x < PO16)) {
        ...
        hi_hi = idiv_PO4((uint32_t)x_div_PO8);
        hi_lo = (uint32_t)(x_div_PO8 - hi_hi * PO4);
        ...
        mid_hi = idiv_PO4((uint32_t)mid);
        mid_lo = (uint32_t)(mid - mid_hi * PO4);
        buf_mid = encode_ten_thousands(mid_hi, mid_lo) + ZERO_CHARS;

        out = itoa_ten_thousand(out, (uint32_t)hi);
...
char *itoa_u32_an(int32_t val, char *buf) {
    return _itoa_u32_an(val, buf);
}

char *itoa_u64_an(uint64_t val, char *buf) {
    return _itoa_u64_an(val, buf);
}

char *itoa_i32_an(int32_t val, char *buf) {
    int neg = val < 0;
    *buf = '-';
    return _itoa_u32_an((uint32_t)(neg ? -val : val), buf + neg);
}

char *itoa_i64_an(int64_t val, char *buf) {
    int neg = val < 0;
    *buf = '-';
    return _itoa_u64_an((uint64_t)(neg ? -val : val), buf + neg);
}

/* benckmark config */
int itoa_an_available_32 = 1;
int itoa_an_available_64 = 1;

#else
#include <stdint.h>
char *itoa_u32_an(uint32_t value, char *buffer) { return buffer; }
char *itoa_i32_an(int32_t value, char *buffer) { return buffer; }
char *itoa_u64_an(uint64_t value, char *buffer) { return buffer; }
char *itoa_i64_an(int64_t value, char *buffer) { return buffer; }

/* benckmark config */
int itoa_an_available_32 = 0;
int itoa_an_available_64 = 0;
#endif

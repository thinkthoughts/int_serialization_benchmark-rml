...
bool test_all() {
    // technically, we only care about from 1000 to 9999
    char buffer[4];
    for (int i = 1100; i < 1199; ++i) {
        write_four_digits_10000(buffer, i);
        if(buffer[0] < '0' || buffer[0] > '9') abort();
        if(buffer[1] < '0' || buffer[1] > '9') abort();
        if(buffer[2] < '0' || buffer[2] > '9') abort();
        if(buffer[3] < '0' || buffer[3] > '9') abort();
        if(buffer[0] - '0' != i / 1000) abort();
        if(buffer[1] - '0' != (i / 100) % 10) abort();
        if(buffer[2] - '0' != (i / 10) % 10) abort();
        if(buffer[3] - '0' != i % 10) abort();
    }
    return true;
}

int main() {
    printf("%s\n", test_all() ? "All tests passed." : "Some tests failed.");
}